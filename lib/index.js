...
function plugin(opts) {
  ...
  opts.tagAsTitle = opts.tagAsTitle || false;
  opts.pageMetadata = opts.pageMetadata || false;
  ...
  return function(files, metalsmith, done) {
    ...
    Object.keys(tagList).forEach(function(tag) {
      if (typeof tag == 'undefined' || tag.length == 0) return
      ...
      for (var i = 0; i < numPages; i++) {
        ...
        if (opts.tagAsTitle == true) {
          page.title = tag.replace(/-/g, ' ')
        }
        
        if (opts.pageMetadata != false) {
          Object.keys(opts.pageMetadata).forEach(function(meta) {
            page[meta] = page.pagination[meta] = opts.pageMetadata[meta]
          })
        }
